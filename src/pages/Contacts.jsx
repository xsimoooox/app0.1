...
export default function Contacts() {
  ...
  const handleToggleSimulateGlove = () => {
    let idx = 0;
    const phrases = ["Bonjour", "Comment vas tu", "Moi ca va", "J'ai fini la tache", "A bientot"];
    ...
    step();
    simGloveIntervalRef.current = setInterval(step, 4000);
  };
    ...
    >
      ...
      {screen === 'detail' && activeContact && (
        <div className="flex-grow flex flex-col p-4 overflow-y-auto">
          ...
          <button
            onClick={launchCall}
            className="w-full h-12 bg-[#16a34a] hover:bg-emerald-600 text-white rounded-[12px] font-extrabold text-[12px] shadow-md shadow-emerald-700/10 flex items-center justify-center gap-2 tracking-wide uppercase active:scale-[0.98] transition-transform cursor-pointer mb-5 shrink-0"
          >
        ...
        <div className="fixed inset-0 bg-[#F0F2F5] z-[9999] flex flex-col justify-between font-sans overflow-hidden text-[#1F2937] select-none max-w-md mx-auto h-[calc(100dvh-80px)] bottom-[80px] animate-fade-in">
          ...
          <ZoneDActionBar
            ...
            onMicro={() => {
              ...
              showToast(microActive ? '🔇 Son interlocuteur coupé' : '🎤 Son interlocuteur activé');
            }}
