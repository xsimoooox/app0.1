...
function getContact(id) {
  return HEARING_CONTACTS.find((contact) => contact.id === id) || HEARING_CONTACTS[0];
}
...
function ContactList() {
  ...
  useEffect(() => {
    ...
    const contact = HEARING_CONTACTS.find((c) => getContactPhone(c) === phone);
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (!cleanQuery) return HEARING_CONTACTS;
    return HEARING_CONTACTS.filter((contact) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[9px] text-[#777777] font-semibold flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="w-[7px] h-[7px] rounded-full inline-block" style={{ backgroundColor: statusColor[contact.status] }} />
                  <span>{contact.statusLabel}</span>
                </div>
              </div>
              {(() => {
                const callUi = getEntendantCallState(
                  getRealtimeStatus(getContactPhone(contact), contact.status),
                );
                return (
                  <span
                    role="button"
    ...
    <div className="w-full max-w-md mx-auto min-h-screen bg-[#f5f5f5] text-[#111111] pb-[88px] select-none animate-fade-in">
      ...
      <div className="px-3.5">
        ...
        <div className="mt-4 rounded-[12px] border border-[#e5e5e5] bg-[#fafafa] p-3 space-y-3">
          {['Nom complet', 'Téléphone', 'Email', 'Ville'].map((label) => (
            <label key={label} className="block">
              <span className="text-[9px] font-bold text-[#777777] uppercase tracking-[0.5px]">{label}</span>
              <input className="mt-1 w-full h-10 rounded-[9px] bg-[#ffffff] border border-[#e0e0e0] px-3 text-[11px] outline-none text-[#333333]" />
            </label>
          ))}
        </div>
    ...
    <div className="w-full max-w-md mx-auto min-h-screen bg-[#f5f5f5] text-[#111111] pb-[88px] select-none animate-fade-in">
      ...
      <section className="bg-[#f0f0f8] px-3.5 pt-5 pb-4 flex flex-col items-center text-center">
        ...
        <div className="flex items-center gap-1.5 mt-2">
          <span className="w-[7px] h-[7px] rounded-full" style={{ backgroundColor: statusColor[contact.status] }} />
          <span className="text-[10px] text-[#22c55e] font-bold">{contact.statusLabel}</span>
        </div>
      </section>
      ...
      <button
        ...
        disabled={getRealtimeStatus(getContactPhone(contact), contact.status) !== 'online'}
      ...
      <section className="grid grid-cols-2 gap-1.5 mx-3.5 mt-[10px]">
        {[
          { Icon: History, color: '#666666', label: 'Historique' },
          { Icon: Star, color: '#fbbf24', label: 'Favoris' },
          { Icon: Share2, color: '#666666', label: 'Partager' },
          { Icon: Trash2, color: '#ef4444', label: 'Supprimer' },
